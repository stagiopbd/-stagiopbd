...
router.post(
    ...
    [
        check('cpf')
            .not()
            .isEmpty()
            .withMessage('Physician Cpf cannot be empty.'),
        check('cpf')
            .isNumeric()
            .isLength(11)
            .withMessage('CPF must be a numeric value with 11 digits'),
        check('cpf').custom(cpf => {
            const tempCpf = ('' + cpf).toLowerCase().replace(new RegExp('[0-9]{11}', 'g'), '');
        ...
        check('name')
            .not()
            .isEmpty()
            .withMessage('Physician Name cannot be empty'),
        check('birthDate')
            .not()
            .isEmpty()
            .withMessage('Physician Birthdate cannot be empty.'),
        ...
        check('gender')
            .not()
            .isEmpty()
            .withMessage('Physician Gender cannot be empty.'),
        check('address')
            .not()
            .isEmpty()
            .withMessage('Physician Address cannot be empty.'),
        check('phone')
            .not()
            .isEmpty()
            .withMessage('Physician Phone cannot be empty.'),
        check('crm')
            .not()
            .isEmpty()
            .withMessage('Physician Crm cannot be empty.'),
        ...
        check('specialties').custom(async (specialties, { req }) => {
            const specialtiesObj = await req.app.locals.mysqlDb
                .getRepository(MedicalSpecialty)
                .find({
                    where: {
